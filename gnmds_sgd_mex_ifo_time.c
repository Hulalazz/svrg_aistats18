/*
 * gnmds_sgd_mex_ifo.c - GNMDS for ordinal embedding with SGD in MATLAB External Interfaces
*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta_int, eta, eta_par, sum_residel, sum, *duration, *X_int, *X_new, *X_old, *X_temp, *train, *test, *stoch_grd, *train_error, *test_error, *X_norm;
	...
	for (n = 0; n < N; ++n)
	{
		for (d = 0; d < dim; ++d)
		{
			...
			*(X_new+t+n*svrg_iter+d*N*svrg_iter) = *(X_int+n+d*N);
		}
	}

	while (s < sgd_iter && relative_residual > 1e-7)
	{
		start = clock();
		*(index+0) = 0;
		*(index+1) = 0;
		*(index+2) = 0;
		for (i = 0; i < num_train; ++i)
		{
			...
			*(index+2) = (int) (*(train+p+2*num_train));
			...
			for (d = 0; d < dim; ++d)
			{
				*(X_temp+(*(index+0))+d*N) -= eta*(*(stoch_grd+0+d*3));
				*(X_temp+(*(index+1))+d*N) -= eta*(*(stoch_grd+1+d*3));
				*(X_temp+(*(index+2))+d*N) -= eta*(*(stoch_grd+2+d*3));
			}
		}
		finish = clock();
		*(duration+t) += (double)(finish - start) / CLOCKS_PER_SEC;
		for (n = 0; n < N; ++n)
		{
			for (d = 0; d < dim; ++d)
			{
				...
				sum_residel += pow(((*(X_temp+n+d*N))-(*(X_old+n+d*N))), 2);
				*(X_old+n+d*N) = *(X_temp+n+d*N);
			}
		}
		...
		if (s%count == 0)
		{
			evaluat_error(X_temp, train, test, label, train_error, test_error, X_norm, error_type, num_train, num_test, N, dim, t);
			for (n = 0; n < N; ++n)
			{
				for (d = 0; d < dim; ++d)
				{
					*(X_new+t+n*svrg_iter+d*N*svrg_iter) = *(X_temp+n+d*N);
				}
			}
			t += 1;
		}
