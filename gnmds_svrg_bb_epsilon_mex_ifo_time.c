/*
 * gnmds_svrg_bb_mex_ifo.c - GNMDS for ordinal embedding with SVRG in MATLAB External Interfaces
*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta_init, sum_residel, sum, epsilon, *eta, *duration, *X_int, *X_new, *X_temp, *X_snapshot_old, *X_snapshot_new, *train, *test, *avg_grad_old, *avg_grad_new, *stoch_grd, *snapshot_grd, *train_error, *test_error;
	...
	frq_iter = (int) mxGetScalar(prhs[11]);
	...
	X_temp = malloc(N*dim*sizeof(*X_temp));
	...
	for (n = 0; n < N; ++n)
	{
		for (d = 0; d < dim; ++d)
		{
			...
			*(X_new+s+n*svrg_iter+d*N*svrg_iter) = *(X_int+n+d*N);
	...
	*(eta+s) = eta_init;
	...
	while (s < svrg_iter)
	{
		...
		for (n = 0; n < N; ++n)
		{
			for (d = 0; d < dim; ++d)
			{
				*(X_temp+n+d*N) = *(X_snapshot_new+n+d*N);
		...
		*(index+1) = 0;
		*(index+2) = 0;
		average_gradient(X_snapshot_new, train, avg_grad_new, num_train, N, dim);
		if (s > 0)
		{
		...
		for (i = 0; i < frq_iter; ++i)
		{
			...
			*(index+2) = (int) (*(train+p+2*num_train));
			object_gradient(X_temp, index, stoch_grd, N, dim);
			object_gradient(X_snapshot_new, index, snapshot_grd, N, dim);
			for (d = 0; d < dim; ++d)
			{
				*(X_temp+(*(index+0))+d*N) -= *(eta+s) * ((*(stoch_grd+0+d*3))-(*(snapshot_grd+0+d*3))) / frq_iter * N;
				*(X_temp+(*(index+1))+d*N) -= *(eta+s) * ((*(stoch_grd+1+d*3))-(*(snapshot_grd+1+d*3))) / frq_iter * N;
				*(X_temp+(*(index+2))+d*N) -= *(eta+s) * ((*(stoch_grd+2+d*3))-(*(snapshot_grd+2+d*3))) / frq_iter * N;
			}
			for (n = 0; n < N; ++n)
			{
				for (d = 0; d < dim; ++d)
				{
					*(X_temp+n+d*N) -= *(eta+s) * (*(avg_grad_new+n+d*N));
				}
			}
		}
		finish = clock();
		*(duration+s) = (double)(finish - start) / CLOCKS_PER_SEC;
		for (n = 0; n < N; ++n)
		{
			for (d = 0; d < dim; ++d)
			{
				...
				sum_residel += pow(((*(X_temp+n+d*N))-(*(X_snapshot_new+n+d*N))), 2);
				*(X_snapshot_old+n+d*N) = *(X_snapshot_new+n+d*N);
				*(X_snapshot_new+n+d*N) = *(X_temp+n+d*N);
				*(X_new+s+n*svrg_iter+d*N*svrg_iter) = *(X_temp+n+d*N);
		...
		sum_residel = 0;
		sum = 0;
		evaluat_error(X_temp, train, test, label, train_error, test_error, error_type, num_train, num_test, N, dim, s);
		//if (*(test_error+s) <= 0.2)
		//{
		//	break;
		//}
