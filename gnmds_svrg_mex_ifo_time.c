...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta, sum_residel, sum, *duration, *X_int, *X_new, *X_snapshot, *X_temp, *train, *test, *avg_grad, *stoch_grd, *snapshot_grd, *train_error, *test_error, *X_norm;
	...
	for (n = 0; n < N; ++n)
	{
		for (d = 0; d < dim; ++d)
		{
			...
			*(X_temp+n+d*N) = *(X_int+n+d*N);
		}
	}
	

	while (s < svrg_iter && relative_residual > 1e-7)
	{
		start = clock();
		for (n = 0; n < N; ++n)
		{
		...
		*(index+2) = 0;
		...
		for (i = 0; i < frq_iter; ++i)
		{
			...
			*(index+2) = (int) (*(train+p+2*num_train));
			...
			for (d = 0; d < dim; ++d)
			{
				*(X_temp+(*(index+0))+d*N) -= eta * ((*(stoch_grd+0+d*3))-(*(snapshot_grd+0+d*3)));
				*(X_temp+(*(index+1))+d*N) -= eta * ((*(stoch_grd+1+d*3))-(*(snapshot_grd+1+d*3)));
				*(X_temp+(*(index+2))+d*N) -= eta * ((*(stoch_grd+2+d*3))-(*(snapshot_grd+2+d*3)));
			}
			for (n = 0; n < N; ++n)
			{
				for (d = 0; d < dim; ++d)
				{
					*(X_temp+n+d*N) -= eta * (*(avg_grad+n+d*N));
				}
			}
		}
		finish = clock();
		*(duration+s) = (double)(finish - start) / CLOCKS_PER_SEC;
		for (n = 0; n < N; ++n)
		{
			for (d = 0; d < dim; ++d)
			{
				...
				sum_residel += pow(((*(X_temp+n+d*N))-(*(X_snapshot+n+d*N))), 2);
				*(X_snapshot+n+d*N) = *(X_temp+n+d*N);
		...
		sum_residel = 0;
		sum = 0;
		evaluat_error(X_temp, train, test, label, train_error, test_error, X_norm, error_type, num_train, num_test, N, dim, s);
		//if (*(test_error+s) <= 0.2)
		//{
		//	break;
		//}
