...
while iter < batch_iter
	...
	old_C = C;
	[C, G, slack] = gnmds_x_grad(temp_X(:), N, no_dims, train_triplets, 'hinge', lambda);
	...
	temp_X = temp_X - (eta ./ no_train * N) .* reshape(G, [N no_dims]);
	...
	if ~rem(iter, no_repeat+2)
		...
		sum_X = sum(temp_X .^ 2, 2);
		D = bsxfun(@plus, sum_X, bsxfun(@plus, sum_X', -2 * (temp_X * temp_X')));
