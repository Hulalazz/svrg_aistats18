...
void barzilai_borwein_step_size(double *X_old, double *X_new, double *eta, double *grad_old, double *grad_new, double frq_iter, double epsilon, int N, int dim, int s, int flag)
{
	...
	if (flag == 2)
	{
		*(eta+s) = 1/frq_iter*(numerator)/(abs(denominator)+epsilon*numerator);
	}
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, *eta, eta_init, epsilon, sum_residel, sum, *duration, *X_int, *X_new, *X_snapshot_old, *X_snapshot_new, *train, *test, *avg_grad_old, *avg_grad_new, *stoch_grd, *snapshot_grd, *train_error, *test_error;
	...
	*(eta+s) = eta_init;
	...
	while (s < svrg_iter && relative_residual > 1e-6)
	{
		start = clock();
		for (n = 0; n < N; ++n)
		{
