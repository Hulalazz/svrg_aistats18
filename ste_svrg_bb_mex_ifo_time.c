/*
 * ste_svrg_bb_mex_ifo.c - GNMDS for ordinal embedding with SVRG in MATLAB External Interfaces
*/
...
double barzilai_borwein_step_size(double *X_old, double *X_new, double *grad_old, double *grad_new, double frq_iter, int N, int dim);
...
double barzilai_borwein_step_size(double *X_old, double *X_new, double *grad_old, double *grad_new, double frq_iter, int N, int dim)
{
	double eta, denominator, numerator;
	int n, d;
	n = d = 0;
	eta = denominator = numerator = 0;
	for (n = 0; n < N; ++n)
	{
		for (d = 0; d < dim; ++d)
		{
	...
	eta = (1/frq_iter)*((numerator+0.0000001)/(denominator+0.0000001));
	return eta;
}
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta, eta_init, sum_residel, sum, *duration, *X_int, *X_new, *X_snapshot_old, *X_snapshot_new, *train, *test, *avg_grad_old, *avg_grad_new, *stoch_grd, *snapshot_grd, *train_error, *test_error;
	...
	while (s < svrg_iter && relative_residual > 1e-5)
	{
		start = clock();
		for (n = 0; n < N; ++n)
		{
		...
		*(index+2) = 0;
		...
		for (i = 0; i < frq_iter; ++i)
		{
			...
			*(index+2) = (int) (*(train+p+2*num_train));
			...
			for (d = 0; d < dim; ++d)
			{
				*(X_new+(*(index+0))+d*N) -= eta * ((*(stoch_grd+0+d*3))-(*(snapshot_grd+0+d*3))) / frq_iter * N;
				*(X_new+(*(index+1))+d*N) -= eta * ((*(stoch_grd+1+d*3))-(*(snapshot_grd+1+d*3))) / frq_iter * N;
				*(X_new+(*(index+2))+d*N) -= eta * ((*(stoch_grd+2+d*3))-(*(snapshot_grd+2+d*3))) / frq_iter * N;
			}
			for (n = 0; n < N; ++n)
			{
				for (d = 0; d < dim; ++d)
				{
					*(X_new+n+d*N) -= eta * (*(avg_grad_new+n+d*N));
				}
			}
		}
		finish = clock();
