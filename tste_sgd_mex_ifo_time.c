/*
 * tste_sgd_mex_ifo.c - t-STE for ordinal embedding with SGD in MATLAB External Interfaces
*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, alpha, eta, eta_int, eta_par, sum_residel, sum, *duration, *X_int, *X_new, *X_old, *train, *test, *stoch_grd, *train_error, *test_error;
	...
	alpha = mxGetScalar(prhs[8]);
	eta_int= mxGetScalar(prhs[9]);
	eta_par = mxGetScalar(prhs[10]);
	scheduling_type = (int) mxGetScalar(prhs[11]);
	sgd_iter = (int) mxGetScalar(prhs[12]);
