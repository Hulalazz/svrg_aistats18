/*
 * tste_svrg_mex_ifo.c - t-STE for ordinal embedding with SVRG in MATLAB External Interfaces
*/
...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[])
{
	...
	double relative_residual, eta, alpha, sum_residel, sum, *duration, *X_int, *X_new, *X_snapshot, *train, *test, *avg_grad, *stoch_grd, *snapshot_grd, *train_error, *test_error;
	...
	frq_iter = (int) mxGetScalar(prhs[10]);
	svrg_iter = (int) mxGetScalar(prhs[11]);
	error_type = (int) mxGetScalar(prhs[12]);
	...
	while (s < svrg_iter && relative_residual > 1e-5)
	{
		...
		*(index+1) = 0;
		*(index+2) = 0;
		average_gradient(X_snapshot, train, avg_grad, num_train, N, dim, alpha);
		for (i = 0; i < frq_iter; ++i)
		{
			...
			*(index+2) = (int) (*(train+p+2*num_train));
			...
			object_gradient(X_snapshot, index, snapshot_grd, N, dim, alpha);
			for (d = 0; d < dim; ++d)
			{
